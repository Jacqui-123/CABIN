...
#could be due to WQ data or other data?
...
#Keep going, because it's likely just just due to other data sources. Going forwards make sure there are the same unique_id's for the benthic data as after the study-benthic join. (n=12028 )


```

Finally, join the study data and benthic data by the newly created Unique_Id column
```{r}
...
colSums(is.na(df_both_samp)) #We still have 2863 NAs in Subsample and none in Status. 
...
num_ref_sites <- sqldf( " select Status, Unique_Id_Sample
                     from df_both_samp Where Status Like '%Refere%' 
...
num_test_sites <- sqldf("select Status, Unique_Id_Sample
                         from df_both_samp where Status LIKE 'Test'
                         Group by Unique_Id_Sample")
...
Reg2 <- sqldf( " Select * from df_both_samp where Region_Name LIKE 'St. Lawrence Drainage Area'  " )
...
df_no_test <- df_both %>%
 replace_na(list(Status = "Unknown", Subsample = "Unknown")) %>% 
  ...
  dplyr::select(Unique_Id_Sample, Region_Name, Subsample, Status, Latitude, Longitude) %>%
  distinct() %>%
...
length(unique(df_no_test$Unique_Id_Sample)) #5889
length(unique(df_test$Unique_Id_Sample)) #7510
#these add to 13399
...
Graph of percent sorted, by region, of reference and potential reference sites, without the Arctic Drainage Area outlier.
```{r}
...
Graph of percent sorted, by region, of only test sites, without outliers
...
##Part 2: Is there a difference between percent sorted, by longitude (E-> W)? 
...
df_no_test %>%
    filter(Subsample <= 100) %>%
ggplot() +
  geom_point(aes(x = Longitude, y = Subsample, colour = Region_Name) ) +
    scale_x_continuous("Longitude") +
  scale_y_continuous("Subsample")
  labs(x = "Longitude", y= "Percent Sorted") +
  ggtitle('Percent Sorted vs Longitude  \n(Reference and Potential Reference Sites)') +
      scale_fill_manual(values=cbPalette) +
theme(legend.position="none" ) +
