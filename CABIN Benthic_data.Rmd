...
**Note: many sites don't have Protocol listed at all. For those, I kept them if "Type" == River. Should double check that these are actually streams.There are ~1375, mostly Arctic. 

-Here is a DF of those sites: 
...
Sanity check after merging - count number of test, potential test, and reference sites there are. These should add to 7058 (they do!) and there should be no NAs for Status column

```{r, echo = FALSE, include=FALSE}

length(unique(df_all$Unique_Id)) #7058

num_ref_sites <- sqldf( "select Status, Unique_Id
                     from df_all Where Status Like '%Refere%' ") #3455


num_test_sites <- sqldf("select Status, Unique_Id
                         from df_all where Status LIKE 'Test' ") #3603
